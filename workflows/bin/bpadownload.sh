...
if [ x"$CKAN_API_TOKEN" = "x" ]; then
  if [ x"$CKAN_API_KEY" = "x" ]; then
    ...
    echo "https://data.bioplatforms.com/user/ka6418"
    echo
...
if [ ! -f tmp/bpa_d155e1b1_20250613T0109_urls.txt ]; then
  echo "tmp/bpa_d155e1b1_20250613T0109_urls.txt not found"
  echo
...
function download_data()
{
  ...
  if ! file_checks $URLS $MD5 ; then
    echo "Missing or unreadable files for $ANNOTATION — skipping $ANNOTATION download."
    return 0
  fi
...
download_data tmp/bpa_d155e1b1_20250613T0109_urls.txt tmp/bpa_d155e1b1_20250613T0109_md5sum.txt main
if [ "$OPTIONAL_DOWNLOAD" = true ] ; then
  download_data tmp/bpa_d155e1b1_20250613T0109_urls_optional.txt tmp/bpa_d155e1b1_20250613T0109_md5sum_optional.txt optional
fi