//since this is result producing step, we can generate an SQL command channel to simply processes - could be optional channel
//add a when statement to stop
...
process longreadstats {
    ...
    output:
    tuple val (sample), val (tech), val (runid), path("*stats*.csv"), path("*freq*.csv"), emit : csv
